...
class WeatherDTO {
    constructor(temperature, humidity, description, icon) {
        this.temperature = temperature;
        this.humidity = humidity;
        this.description = description;
        this.icon = `https://openweathermap.org/img/wn/${icon}@2x.png`; // 取得對應的 icon URL
    }
}
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // 取得 API 回應
        const weather = response.data.weather[0]; // 取得 weather 物件
        ...
        const weatherData = new WeatherDTO(
            ...
            weather.description,
            weather.icon // 這裡是 API 回應的 icon 代碼
        );
